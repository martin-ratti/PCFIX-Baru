...
export default function CartView() {
  ...
  const addToast = useToastStore(s => s.addToast); // Hook para mensajes

  const subtotal = items.reduce((total, item) => total + item.price * item.quantity, 0);
  const isAdmin = user?.role === 'ADMIN';

  // Lógica segura para incrementar
  const handleIncrease = (itemId: string) => {
    const item = items.find(i => i.id === itemId);
    if (item) {
      if (item.quantity >= item.stock) {
        addToast(`Solo hay ${item.stock} unidades disponibles de este producto`, 'error');
        return;
      }
  ...
  const handleCheckout = async () => {
    if (!isAuthenticated) {
      addToast('Debes iniciar sesión para finalizar la compra', 'info');
      await navigate('/login');
      return;
    }
    
    alert('¡Funcionalidad de compra en construcción! (Backend pendiente)');
  };
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <button 
          onClick={() => navigate('/')}
          className="bg-primary text-light font-bold py-3 px-8 rounded-lg hover:bg-opacity-90 transition-colors"
        >
          Ir a la tienda
        </button>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row items-center bg-white p-4 rounded-lg shadow-md gap-4 hover:shadow-lg transition-shadow duration-300">
              ...
              <div className="flex-grow text-center sm:text-left">
                ...
                {item.stock < 5 && <p className="text-xs text-red-500 font-bold mt-1">¡Últimas {item.stock} unidades!</p>}
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-gray-300 rounded-lg bg-gray-50">
                  ...
                  <button 
                    ...
                    className={`px-3 py-1 transition-colors font-bold ${item.quantity >= item.stock ? 'text-gray-300 cursor-not-allowed' : 'text-gray-600 hover:bg-gray-200'}`}
                  >
                    +
                  </button>
                </div>
      ...
      <div className="lg:col-span-1">
        <div className="bg-white p-6 rounded-lg shadow-md sticky top-24 border border-gray-100">
          ...
          <div className="flex justify-between font-bold text-2xl border-t border-gray-200 pt-4 mt-4 text-primary">
            <span>Total</span>
            <span>${subtotal.toLocaleString('es-AR')}</span>
          </div>
          ...
          {isAdmin ? (
            ...
            <button 
                ...
                className="w-full mt-6 bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 transition-all shadow-lg shadow-green-200 hover:shadow-green-300 active:scale-[0.98]"
            >
          ...
          <button 
            onClick={() => navigate('/')} 
            className="w-full mt-3 border-2 border-primary text-primary font-bold py-3 rounded-lg hover:bg-gray-50 transition-colors"
          >
            Seguir Comprando
          </button>
          ...
          <button 
            onClick={clearCart} 
            className="w-full mt-4 text-sm text-red-400 hover:text-red-600 hover:underline text-center"
          >
            Vaciar Carrito
          </button>
        </div>
