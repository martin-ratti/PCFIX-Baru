...
export default function SaleDetailModal({ isOpen, sale, autoFocusDispatch, onClose, onApprove, onReject, onDispatch }: SaleDetailModalProps) {
    ...
    const handleDispatch = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.PUBLIC_API_URL || 'http://localhost:3002/api'}/sales/${sale.id}/dispatch`, {
                method: 'PUT',
                headers: {
    ...
    const handleCreateZipnovaShipment = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.PUBLIC_API_URL || 'http://localhost:3002/api'}/sales/${sale.id}/create-shipment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] animate-modal-enter">
                ...
                <div className="px-6 py-4 bg-white border-t border-gray-200 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        <button onClick={onClose} className="px-5 py-2 text-gray-600 font-bold hover:bg-gray-100 rounded-lg transition-all active:scale-95">Cerrar</button>
