    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center bg-gray-50 gap-4">
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer bg-white px-3 py-2 rounded border border-gray-200 hover:bg-gray-100">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {products.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500"><span className="px-2 bg-blue-100 text-blue-800 text-xs rounded-full">{p.categoria.nombre}</span></td>
                  <td className="px-6 py-4 text-sm text-gray-500 font-mono">${Number(p.precio).toLocaleString('es-AR')}</td>
                  <td className="px-6 py-4 text-sm"><span className={`px-2 py-1 rounded font-bold text-xs ${p.stock < 5 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{p.stock} u.</span></td>
