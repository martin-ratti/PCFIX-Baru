...
export default function BrandCarousel({ brands }: BrandCarouselProps) {
  // CORRECCIÓN: Ahora se muestra si hay al menos 1 marca
  if (!brands || brands.length === 0) return null;

  return (
    <section className="py-12 bg-white border-t border-gray-200 mb-0">
      <div className="container mx-auto px-6">
         <h2 className="text-xl font-bold text-center mb-8 text-gray-400 uppercase tracking-widest text-xs">
         ...
         <Swiper
            modules={[Autoplay]}
            spaceBetween={40}
            // Si hay pocas marcas, mostramos esa cantidad. Si hay muchas, mostramos 3.
            slidesPerView={Math.min(brands.length, 3)} 
            loop={brands.length > 3} // Solo hace loop si hay suficientes elementos
            speed={4000}
            autoplay={{
                delay: 0,
