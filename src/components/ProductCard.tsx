...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    // CAMBIO: Se añaden las clases para el efecto hover
    <div className="bg-white rounded-lg overflow-hidden shadow-xl transform hover:-translate-y-2 transition-transform duration-300 h-full flex flex-col">
      <a href={`/producto/${product.id}`} className="flex flex-col flex-grow">
        ...
        <div className="p-4 flex flex-col justify-between flex-grow">
          ...
          <button className="mt-4 w-full bg-primary text-light font-bold py-2 px-4 rounded-lg hover:bg-opacity-80 transition-colors">
            Agregar al Carrito
          </button>
        </div>
      </a>
